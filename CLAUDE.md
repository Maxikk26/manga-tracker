...
**A pull request body is written in English and follows `.github/pull_request_template.md`.** Read that file before writing one — it is a form with instructions inside an HTML comment, not a draft. Real `##` headings, a blank line before every list, table and fence, bullets rather than paragraphs, and short: twenty lines is a good body, ninety is a symptom, because the commits already carry the reasoning and `docs/` carries the decisions. Its `## Verified` section asks which guard you broke on purpose and what failed; answer it by actually breaking one, since a green suite proves only that nothing you wrote is checked. **Open the pull request yourself with `gh pr create --body-file`, then hand over its URL as a full clickable `https://` link.** The owner reviews and merges from the GitHub UI; they should only have to confirm, not assemble. Changed on 2026-09-08 at the owner's request — this rule previously asked for three things (compare link, title, finished body) for them to paste in by hand, which is strictly more work for the same result. Pass the body through `--body-file` and not inline, so that a body with backticks, tables and blank lines survives the shell intact.

The link is still the part that keeps getting dropped, and it is now the *only* thing handed over, so dropping it leaves nothing. It went missing twice while the rule sat in a memory note, and a third time on 2026-09-08 — a PR URL written without the `https://` scheme, which the terminal does not turn into a link, so the owner had to copy it by hand. Full scheme, always.
