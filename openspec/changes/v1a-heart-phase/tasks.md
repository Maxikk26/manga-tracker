...
> **Attempt 5 status**: the loader engine (validation, per-row write, D14, re-run reuse) and the CSV template landed. **Deferred to attempt 6** (disclosed, 200-line ceiling): `ensure_site`, `config.py`, `logging_setup.py`, `cli.py`, `.env.example`, non-blocking warnings, the skip-invalid override, and the duplicate-slug-in-file check. `site_id`/`site_base_url` are accepted as explicit parameters by `load_seed` in the meantime.

- [ ] 3.1 Add `ensure_site(conn, name, base_url)` to `manga_tracker/storage/db.py`: `ON CONFLICT(name) DO UPDATE SET base_url=excluded.base_url, updated_at=?` (not `INSERT OR IGNORE` — must refresh a stale host per SRC §9 playbook). **Deferred to attempt 6** — drafted and measured over budget in attempt 5; tests currently seed the `sites` row directly.
- [ ] 3.2 Create `manga_tracker/config.py`: `load_config()` reads `os.environ` into frozen dataclasses; collects and reports ALL missing/invalid vars in one failure; `seed` subcommand does not require `TELEGRAM_BOT_TOKEN`. **Deferred to attempt 6.**
- [ ] 3.3 Create `manga_tracker/logging_setup.py`: stdlib `logging`, one stdout `StreamHandler`, plain text UTC, `LOG_LEVEL` env; handler-level filter defaulting `job_name`/`run_id` to `-` outside a run. **Deferred to attempt 6.**
- [ ] 3.4 Create `manga_tracker/cli.py` (the only composition root): argparse entrypoint wiring `run`, `seed`, `test-telegram`, `run-job {feed_check,active_sweep}`; every invocation opens a connection (`ensure_schema`) and calls `ensure_site(conn, "manganato", client.BASE_URL)`. **Deferred to attempt 6** — needs 3.1/3.2/3.3 first.
- [ ] 3.5 Create `.env.example` documenting every env var `config.py` consumes. **Deferred to attempt 6** — needs 3.2 first.
...
- [ ] 3.11 Write `tests/seed/test_loader.py`: every SEED error/warning, slug extraction across ficha/chapter/www/trailing-slash/query/fragment, progress never derived from URL, re-run idempotency, zero-chapter slug reported and discarded whole, not-found/unexpected row fully discarded. **Partial** (3 tests landed, cut per the disclosed priority order): dry-run-blocks-write (`test_invalid_row_blocks_entire_load_and_writes_nothing`), D14 zero-chapters discard (`test_zero_chapters_row_reported_and_discarded_whole`), slug extraction across ficha/chapter+www+query+fragment (`test_extract_slug_tolerates_ficha_and_chapter_url_variants`, 2 variants instead of the full 6-way matrix). Deferred: warnings, duplicate-slug-in-file, not-found/unexpected-discards-and-continues, re-run-idempotency, `ensure_site`-refresh (needs 3.1 first).
- [ ] 3.12 Write `tests/storage/test_ensure_site.py`: `ensure_site` refreshes `base_url` on conflict. **Deferred to attempt 6** — needs 3.1 first.
- [x] 3.13 Extend `tests/test_architecture.py`: add the `seed/**` half of rule 4 (may import `sources.contracts` but never `sources.manganato`). Done in unit 1 (see 1.16 note) — verified unmodified against attempt 5's new `seed/loader.py` and `storage/repositories.py`.
- [x] 3.14 Verify with `git check-ignore --stdin`: `seed-plantilla.csv` is versioned (exit 1, not ignored), `data/seed.csv` and `data/manga-tracker.db` are ignored (exit 0 each, matched by `.gitignore:31:*.csv` and `.gitignore:20:data/*` respectively).
