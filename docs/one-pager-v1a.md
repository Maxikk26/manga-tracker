...
1. **Fase corazón**: esquema SQLite + seed manual (<20) + cliente de fuente + `feed_check` + **`active_sweep`** + digest Telegram + deploy en Docker. Hito: primera notificación real.
2. **Fase red de seguridad**: `onhold_sweep` + heartbeat semanal + aviso de slug muerto por Telegram. Hito: un ciclo semanal completo corrido solo.
...
- **Conjunto de dependencias**: **3 directas — curl-cffi, beautifulsoup4, APScheduler — y 9 paquetes instalados** en total. El número que importa es el segundo, no el primero. `curl-cffi` queda capeado en `<0.15`: la 0.15.0 agregó `rich` como dependencia incondicional, lo que arrastra `markdown-it-py`, `mdurl` y `Pygments` — un renderizador de Markdown y un resaltador de sintaxis dentro de una imagen headless. Con el cap el cierre es 9; sin él, 13. Verificado el 2026-07-28 contra la fuente real: `curl-cffi` 0.14.0 conserva los 19 targets de Chrome y el feed responde 200 sin challenge de Cloudflare, así que el cap no sacrifica la garantía anti-bot. **El cap es una preferencia, no un muro**: se levanta si hace falta un target de impersonation que la 0.14 no tenga (playbook del §9 de la fuente) o si aparece un aviso de seguridad de libcurl. Ambas cosas pesan más que el tamaño del cierre.
