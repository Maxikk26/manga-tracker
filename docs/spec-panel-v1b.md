...
Versión 1.2 — 2026-08-19. Depende de `one-pager-v1a.md` (v1.14), `spec-modelo-de-datos.md` (v1.9) y `decision-arquitectura-v1b.md` (v1.2).
...
- **La fase 3 se entrega en tres PRs encadenados** (`openspec/changes/panel-v1b-fase-3`): el primero cierra la frontera (`web` no llega a la fuente, ni siquiera indirectamente) y los contratos de `intake`; el segundo agrega el escritor de repositorio, `PastedUrlIntake` y los dos endpoints; el tercero agrega el modal del frontend. `POST /api/mangas` y `POST /api/mangas/preview` no existen todavía mientras el primer PR es el único desplegado — la corrección del costo por alta de esta versión describe el flujo completo, no lo ya entregado.
