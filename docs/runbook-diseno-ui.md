...
Versión 1.2 — 2026-09-04. Depende de `spec-panel-v1b.md` (v1.13).
...
- ~~**El tono/tamaño de la pastilla "+N" sigue reservado al dueño**, detrás de las custom properties de `.behind-pill`. Seguía siendo la mejor candidata a primera comparación real, y el 2026-08-30 se descubrió algo que la hace más urgente que estética: **el prototipo de la fase 5 dejó de dibujarla**. `behind()` sólo decide si una tarjeta está "Al día" y el número nunca se muestra, mientras producción sí lo pinta (`frontend/src/components/BookmarkCard.tsx`). Antes de discutir su tono hay que decidir si vuelve.~~ **Cerrado el 2026-09-04, y la respuesta fue que no vuelve.** Decisión del dueño dentro del pase de la fase 5 (decisión 4, slice 1): la pastilla no se ajustó de tono ni de tamaño, se **retiró**. La razón es la medida de siempre — se disparaba en 18 de 18 filas de "Leyendo", así que no distinguía una tarjeta de otra. `BookmarkCard.tsx` dejó de dibujarla, `styles.css` perdió sus cuatro custom properties, y queda un test que **falla si alguien la restaura** (`BookmarkCard.test.tsx`, "the backlog-count pill"), puesto ahí porque su ausencia parece un defecto y no lo es. Lo que este runbook había anotado como divergencia entre el prototipo y producción no era una regresión del prototipo: era la decisión llegando antes que el documento. Registro completo en `spec-panel-v1b.md` v1.13.
...
- **1.2 — 2026-09-04.** **Se cierra el pendiente de la pastilla "+N", y no como el ajuste de tono que este documento esperaba: la pastilla se retiró.** Decisión del dueño dentro del pase de la fase 5 (decisión 4, slice 1), desplegada el 2026-09-04. La v1.1 había registrado como hallazgo urgente que el prototipo dejara de dibujarla mientras producción sí la pintaba, y planteaba la pregunta correcta —"antes de discutir su tono hay que decidir si vuelve"—; la respuesta fue **no**, por la misma razón medida que la hizo sospechosa desde el principio: se disparaba en 18 de 18 filas de "Leyendo", así que no distinguía una tarjeta de otra. Con eso la divergencia que este runbook anotó deja de leerse como regresión del prototipo y pasa a lo que era, la decisión llegando antes que el documento. `BookmarkCard.tsx` ya no la dibuja, `styles.css` perdió sus cuatro custom properties y un test falla si alguien la restaura. **El protocolo no cambia** en nada con esta revisión: sigue sin correrse una comparación completa de tres herramientas, y sigue en pie para la primera pantalla que la pague. Pin actualizado: `spec-panel-v1b.md` v1.10 → **v1.13**, la versión que registra el cierre de V1b.
